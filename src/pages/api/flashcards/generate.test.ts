...
describe("POST /api/flashcards/generate", () => {
  ...
  describe("Happy Path - Successful Generation", () => {
    it("should successfully generate flashcards with valid input", async () => {
      ...
      expect(mockGenerateFlashcards).toHaveBeenCalledWith(validInput.text, expect.any(String));
    });
    ...
    it("should trim whitespace from input text", async () => {
      ...
      expect(result.status).toBe(200);
      expect(result.body.success).toBe(true);
      expect(mockGenerateFlashcards).toHaveBeenCalledWith("Valid text with whitespace", expect.any(String));
    });
  ...
  describe("OpenRouter Service Errors", () => {
    it("should handle missing API key configuration", async () => {
      // Arrange
      mockGenerateFlashcards.mockRejectedValue(new Error("OPENROUTER_API_KEY is not set in environment variables."));
    ...
    it("should handle API connection errors", async () => {
      // Arrange
      mockGenerateFlashcards.mockRejectedValue(new Error("Failed to connect to OpenRouter API"));
    ...
    it("should handle invalid API response structure", async () => {
      // Arrange
      mockGenerateFlashcards.mockRejectedValue(new Error("Invalid API response structure: no choices found."));
  ...
  describe("Response Transformation", () => {
    ...
    it("should preserve unicode characters in flashcard content", async () => {
      ...
      const mockAIResponse: StructuredResponse = {
        ...
        flashcards: [{ question: "Co to jest ąćęłńóśźż?", answer: "Polski alfabet 😊" }],
      };
      mockGenerateFlashcards.mockResolvedValue(mockAIResponse);
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle text with multiple consecutive spaces", async () => {
      ...
      // Assert
      expect(result.status).toBe(200);
      expect(mockGenerateFlashcards).toHaveBeenCalledWith(textWithSpaces, expect.any(String));
    });
  });
  ...
  describe("Integration-like Scenarios", () => {
    it("should handle complete flow from request to response", async () => {
      ...
      expect(mockGenerateFlashcards).toHaveBeenCalledWith(userText, expect.any(String));
