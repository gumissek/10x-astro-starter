...
const useFolderState = (folderId: string, userId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(
            "Folder nie został znaleziony. Prawdopodobnie został usunięty lub nie masz do niego dostępu."
          );
        }
  ...
  const fetchFlashcards = async (page = 1) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(
            "Folder nie został znaleziony lub nie masz do niego dostępu. Sprawdź, czy folder istnieje i należy do Ciebie."
          );
        }
  ...
  const updateFlashcard = async (flashcardId: string, updates: UpdateFlashcardCommand) => {
    try {
      ...
      setState((prev) => ({
        ...prev,
        flashcards: prev.flashcards.map((card) => (card.id === flashcardId ? updatedFlashcard : card)),
        editingFlashcard: null,
      }));
    } catch (error) {
      setState((prev) => ({
        ...
        {folder.flashcard_count > 0 && folder.flashcard_count < 10 && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-800 text-sm">
              Potrzebujesz co najmniej 10 fiszek, aby rozpocząć sesję nauki. Obecnie masz {folder.flashcard_count}{" "}
              {folder.flashcard_count === 1 ? "fiszkę" : "fiszek"}.
            </p>
          </div>
        )}
      </div>
