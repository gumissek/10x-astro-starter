  ...
  return (
    <Card className="hover:shadow-md transition-shadow group">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg font-semibold text-gray-900 truncate">{folder.name}</CardTitle>
      </CardHeader>
        ...
        {folder.flashcard_count > 0 && folder.flashcard_count < 10 && (
          <div className="w-full text-left">
            <p className="text-xs text-yellow-600">
              Potrzebujesz {10 - folder.flashcard_count} więcej{" "}
              {10 - folder.flashcard_count === 1 ? "fiszki" : "fiszek"} do nauki
            </p>
          </div>
        )}
      </CardFooter>
