...
const ResetPasswordForm: React.FC = () => {
  ...
  const handleFieldChange = useCallback(
    (field: keyof ResetPasswordFormViewModel, value: string) => {
  ...
  const handleBlur = useCallback(
    (field: keyof ResetPasswordFormViewModel) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setValidationErrors({
        general: error instanceof Error ? error.message : "Wystąpił błąd podczas resetowania hasła",
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="space-y-6">
        <div className="space-y-2 text-center">
          ...
          <p className="text-muted-foreground">
            Twoje hasło zostało pomyślnie zaktualizowane. Możesz teraz zalogować się przy użyciu nowego hasła.
          </p>
        </div>

        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <p className="text-sm text-green-700">
            Hasło zostało pomyślnie zmienione. Ze względów bezpieczeństwa zostałeś wylogowany ze wszystkich urządzeń.
          </p>
        </div>
